...
public class SLList {
    ...
    /** Inser item at the given position */
    public void insert(int x, int position) {
    	...
    	IntNode newNode = new IntNode(x,cur.next);
    	cur.next = newNode;
    	size += 1;
    }
    
    /** Reverse the elements in list iteratively */
    public void reverse() {
        IntNode cur = sentinel.next.next;
        IntNode front = sentinel.next;
    	while(sentinel.next.next != null){
            IntNode tail = cur.next;
    ...
    /** Reverse the elements in list recursively */
    public void reverseRecursive() {
        sentinel.next = reverseRecursive(sentinel.next);
    }
    /** Helper function */
    private IntNode reverseRecursive(IntNode n){
        if(n.next == null) {
            return n;
        } else{
            IntNode front = reverseRecursive(n.next);
            n.next.next = n;
            n.next = null;
            return front;
        }
