...
public class Percolation {
    ...
    public Percolation(int N){
        if(N<=0) throw new IndexOutOfBoundsException("In Constructor");
    ...
    private void connectSurrounding(int row, int col){
        ...
        if(row<size-1 && grid[xyTo1D(row+1,col)]==1){
            uf1.union(cur,xyTo1D(row+1,col));
            uf2.union(cur,xyTo1D(row+1,col));
        }
        if(row>0 && grid[xyTo1D(row-1,col)]==1){
            uf1.union(cur,xyTo1D(row-1,col));
            uf2.union(cur,xyTo1D(row-1,col));
        }
        if(col<size-1 && grid[xyTo1D(row,col+1)]==1){
            uf1.union(cur,xyTo1D(row,col+1));
            uf2.union(cur,xyTo1D(row,col+1));
        }
        if(col>0 && grid[xyTo1D(row,col-1)]==1){
            uf1.union(cur,xyTo1D(row,col-1));
            uf2.union(cur,xyTo1D(row,col-1));
        }
    }
