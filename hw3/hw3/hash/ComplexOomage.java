...
public class ComplexOomage implements Oomage {
    ...
    public int hashCode() {
        int total = 0;
        /** Fix the original hash function by adding an "cap" that is the max integer value
         *  divided by 256, and mode the hash code to prevent integer overflow
         */
        int cap = Integer.MAX_VALUE / 256;
        for (int x : params) {
            total = total * 256;
            total = total + x;
            total = total % cap;
